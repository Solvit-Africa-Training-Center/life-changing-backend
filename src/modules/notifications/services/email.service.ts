...
export class EmailService {
  ...
  isEmailEnabled(): boolean {
   ...
   async sendDonationReceipt(email: string, receiptData: DonationReceiptData): Promise<boolean> {
    const subject = receiptData.language === Language.RW
      ? `Inyemezabuguzi Yawe - ${receiptData.receiptNumber}`
      : `Your Donation Receipt - ${receiptData.receiptNumber}`;

    const { html, text } = this.generateDonationReceiptContent(receiptData);
